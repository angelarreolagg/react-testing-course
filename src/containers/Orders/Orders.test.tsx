...
describe("<Orders />", () => {
  ...
  it("renders should be rendered", async () => {
  ...
  it("should render the OrderSummary component for super admin users", async () => {
    mockGetOrders.mockResolvedValue(mockOrders);
    handleRenderOrders("superadmin");

    const { totalOrders } = getSummaryOrders(mockOrders);
    const totalOrdersLabel = await screen.findByTestId("total-orders");
    const totalOrdersValue = totalOrdersLabel.textContent;
